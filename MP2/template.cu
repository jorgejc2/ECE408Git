...
int main(int argc, char **argv) {
  ...
  /* getting the gpu properties to dynamically create tile size */
  // cudaDeviceProp dev_prop;
  // int dev_count;
  // cudaGetDeviceCount(&dev_count);
  // for (int i = 0; i < dev_count; i++) {
  //   cudaGetDeviceProperties(&dev_prop, i);
  //   // std::cout << "Properties of " << i << ": " << dev_prop << '\n';
  //   // printf("Properties of %d: ", i);
  //   // printf(dev_prop);
  //   // printf('\n');
  // }

  /* for now, will have 16x16 blocks */
  dim3 dimGrid(ceil((float)numCColumns/TILE_WIDTH), ceil((float)numCRows/TILE_WIDTH), 1);
  dim3 dimBlock(TILE_WIDTH, TILE_WIDTH, 1);

  wbTime_start(Compute, "Performing CUDA computation");
  //@@ Launch the GPU Kernel here
  matrixMultiply<<<dimGrid, dimBlock>>>(deviceA, deviceB, deviceC, numARows, numAColumns, numBRows, numBColumns, numCRows, numCColumns); // will need a third instantiation param for dynamic shared memory
  ...
  //     ++zero_count1;
  // }
  // printf(" %d zeros", zero_count1);
  // printf("\n\n");
  // /* reads last row */
  // for (int i = 0; i < numCColumns; i++) {
  //   r2 = *(hostC + ((numCRows-1) * numCColumns + i));
  //   printf("%.2f ", r2);
  //   if (r2 == 0.00)
  //     ++zero_count2;
  // }
  // printf(" %d zeros", zero_count2);
